...
const TokenDetails = (props) => {
    ...
    const rewards =
        props.rewards &&
        props.rewards.total &&
        props.rewards.total.length &&
        props.rewards.total[0] &&
        props.rewards.total[0].amount
            ? props.rewards.total[0].amount / 10 ** config.COIN_DECIMALS
            : 0;
    ...
    useEffect(() => {
        (async () => {
            ...
            const {
                ...
                .then((r) => r.json())
                .catch((e) => ({ pool: { bonded_tokens: 0 } }));
            setApr(((+supply * +inflation) / +bonded) * 100);
            const price = await fetch(
                `https://api-osmosis.imperator.co/tokens/v1/${config.COIN_DENOM.toUpperCase()}`,
            )
                .then((r) => r.json())
                .then(([{ price }]) => price)
                .catch((e) => 0);
                ...
                <div className="chip">
                    ...
                    <p>
                        {new Intl.NumberFormat().format(
                            +(aprToApy(apr / 100, blocksInAYear) * 100).toFixed(
                                2,
                            ),
                        )}
                        %
            ...
            <div className="chip_info">
                ...
                <div className="chip">
                    ...
                    <p>
                        $
                        {new Intl.NumberFormat().format(
                            +(supply / 10 ** config.COIN_DECIMALS) * price,
                        )}
                    </p>
                </div>
            </div>
                ...
                <div className="chip">
                    ...
                    <p>
                        {new Intl.NumberFormat().format(
                            +(
                                +(staked / 10 ** config.COIN_DECIMALS) *
                                aprToApy(apr / 100, blocksInAYear)
                            ),
                        )}
                        , $
                        {new Intl.NumberFormat().format(
            ...
            <div className="chip_info">
                ...
                <div className="chip">
                    ...
                    <p>
                        $
                        {new Intl.NumberFormat().format(
                            (staked / 10 ** config.COIN_DECIMALS) * price,
                        )}
                    </p>
                </div>
            ...
            <div className="chip_info">
                ...
                <div className="chip">
                    ...
                    <p>
                        $
                        {new Intl.NumberFormat().format(
                            (available / 10 ** config.COIN_DECIMALS) * price,
                        )}
                    </p>
                </div>
                <div className="buttons_div"></div>
